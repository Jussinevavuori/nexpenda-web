...
import { DeprecatedFileUploaderProps } from "./DeprecatedFileUploader";

export function useDeprecatedFileUploaderController(
  props: DeprecatedFileUploaderProps
) {
  const postTransactions = useStoreActions(
    (_) => _.transactions.massPostTransactions
  );

  const [result, setResult] = useState<
    undefined | null | SpreadsheetReadSheetResult<JsonSpreadsheetTransaction>
  >();
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const readResult = await transactionSpreadsheet.readSheet(e.target, {
      sheetIndex: 0,
    });
