...
import { MoneyAmountComparison } from "../Money/MoneyAmountComparison";
...
export class SmartTransactionFilter {
  ...
  private matchesDateRange(transaction: Transaction) {
    return (
      compareDate(transaction.date, ">=", this.context.startDate) &&
      compareDate(transaction.date, "<=", this.context.endDate)
    );
  }
  ...
  private matchesSearchTerm(transaction: Transaction) {
    ...
    return textSearch(this.search, ...targets);
  }
